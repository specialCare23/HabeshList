...
function get_requirements() {
  ...
  if($php_min <> '') {
    $array['PHP min version'] = array(
      ...
      'solution' => sprintf(__('At least PHP %s is required to run Osclass. You may talk with your hosting to upgrade your PHP version.'), $php_min)
    );
  }
  ...
  if($php_max <> '') {
    $array['PHP max version'] = array(
      ...
      'solution' => sprintf(__('PHP %s is latest version supported by Osclass. You may talk with your hosting to downgrade your PHP version.'), $php_max)
    );
  }
  ...
  if($error_num > 0) {
    ...
    switch ($error_num) {
      ...
      case 2005:
        ...
        break;
      default:
        return array('error' => sprintf(__('Cannot connect to the database. Error: %s - %s'), $error_num, $error_desc));
        break;
    }
  }
...
function display_finish($password) {
  ...
  <div class="row cron-setup"><?php echo sprintf(__("Don't forget to setup %s and %s after installation."), '<a target="_blank" href="https://docs.osclasspoint.com/cron-setup">CRON</a>', '<a target="_blank" href="https://docs.osclasspoint.com/setting-up-a-mail-server">' . __('Mail server') . '</a>'); ?></div>
