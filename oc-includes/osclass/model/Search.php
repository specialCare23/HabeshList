...
class Search extends DAO {
   ...
   * @param bool $expired
   */
  public function __construct($expired = false, $stats = false) {
    parent::__construct();
  ...
  public static function getAllowedColumnsForSorting() {
    return osc_apply_filter('search_list_columns', array('i_price', 'dt_pub_date', 'dt_expiration'));
  }
  ...
  public static function getAllowedTypesForSorting() {
    return osc_apply_filter('search_list_types', array(0 => 'asc', 1 => 'desc'));
  }
  ...
  public function getPremiums($max = 2) {
    ...
    $key = md5(osc_base_url().'Search::getPremiums'.(string)$premium_sql.(string)osc_current_user_locale());
    $found = null;
    $cache = osc_cache_get($key, $found);
  ...
  public function getLatestItems($numItems = 10, $options = array(), $withPicture = false) {
    $key = md5(osc_base_url().'Search::getLatestItems'.(string)$numItems.json_encode($options).(string)$withPicture.(string)osc_current_user_locale());
    $found = null;
