...
HTACCESS;
        ...
        if( $rewriteEnabled ) {
          ...
          $item_rule = str_replace(
            array('{ITEM_ID}','{CATEGORIES}','{ITEM_TITLE}','{ITEM_COUNTRY}','{ITEM_COUNTRY_CODE}','{ITEM_REGION}','{ITEM_CITY}','{ITEM_CITY_AREA}','{ITEM_ZIP}','{ITEM_CONTACT_NAME}','{ITEM_CONTACT_EMAIL}','{ITEM_CURRENCY_CODE}','{ITEM_PUB_DATE}'),
            array('([0-9]+)','.*','.*','.*','.*','.*','.*','.*','.*','.*','.*','.*','.*'),
            $item_url
          );
